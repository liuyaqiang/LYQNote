...
@interface LYQReactiveTableVM ()
{
    ...
    *RACSwitchToLatestStr;
    ...
    NSString *RACTimeOutStr,
    *RACIntervalStr,
    *RACDelayStr;
    
    NSString *RACMVVMTestStr;
}
...
- (void)selectCellWithIndextPath:(NSIndexPath *)indexPath
{
    ...
    if (text == RACSiganalTestStr) {
        ...
        if (self.DelegateSubject) {
            // 有值，才需要通知
            [self.DelegateSubject sendNext:@{@"title":text, @"controller" : @"LYQReactiveUniversalUseViewController"}];
        ...
        if (self.DelegateSubject) {
            // 有值，才需要通知
     [self.DelegateSubject sendNext:@{@"title":text, @"controller" : @"LYQReactiveUniversalUseViewController"}];            [[_redV rac_signalForSelector:@selector(viewDidLoad)] subscribeNext:^(id x) {
    ...
    }else if (text == RACTimeOutStr){
        [self RACTimeOutExample];
    }else if (text == RACIntervalStr){
        [self RACIntervalExample];
    }else if (text == RACDelayStr){
        [self RACDelayExample];
    }else if (text == RACMVVMTestStr){
        if (self.DelegateSubject) {
            // 有值，才需要通知
            [self.DelegateSubject sendNext:@{@"title":text, @"controller" : @"LYQMVVMTestViewController"}];
        }
    }
}
 ...
 */
#pragma mark - 1.9 ReactiveCocoa操作方法之时间。
//timeout：超时，可以让一个信号在一定的时间后，自动报错。
- (void)RACTimeOutExample
{
    RACSignal *signal = [[RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        return nil;
...
#pragma mark - 1.9 ReactiveCocoa操作方法之重复。
//retry重试 ：只要失败，就会重新执行创建信号中的block,直到成功.
- (void)RACRetryExample
{
    __block int i = 0;
    [[[RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
...
- (NSArray *)dataArr
{
    ...
    RACTimeOutStr = @"RACTimeOutTest",
    RACIntervalStr = @"RACIntervalTest",
    RACDelayStr = @"RACDelayTest";
    
    RACMVVMTestStr = @"RACMVVMTest";
    if (!_dataArr) {
        _dataArr = @[
